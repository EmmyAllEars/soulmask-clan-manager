...
- **Talents** — searchable dropdown of all 551 talents loaded from the talent catalog,
  spanning five visual buckets: positive (default), negative defects (red), preferences
  (purple/pink), origins (amber), and titles (blue/grey). Auto-suggests as you type.
  Hard-warns at 6 positive talents max — preferences, origins, and titles don't count
  toward the cap.
...
├── style.css               # styles
├── data/
│   ├── talents.json        # 551 talents catalog (regenerated by tools/generate-talents)
│   └── default_roster.json # 25-tribesman starter roster
├── icons/                  # talent icon WebP files
├── tools/
│   ├── generate-talents/   # Node tools: rebuild talents.json from upstream scrape
